...
class Hex():
    ...
    def GetResult(self, playerjm): # TODO
        """ Get the game result from the viewpoint of playerjm. 
        """
        won, player = self._check_winner()
        if won:
            if player == playerjm:
                return 1.0
        else:
    ...
    def _check_winner(self):
        if self.current == BLUE:
            return self._is_connected(self._left(), self._right(), BLUE), self.current
        return self._is_connected(self._top(), self._bottom(), RED), self.current
